...
def print_item(my_list, i=0):
    ...
    """
    if my_list == []: #basecase
        return #when not explicitely returning something it returns None

    print my_list[i] #prints each item, but started with first
    return print_item(my_list[1:]) #calls itself to print each item while skipping first item since it's already printed in line 17
...
def print_all_tree_data(tree):
    ...
    print tree.data
    for child in tree.children: #basecase is in node.children. Once there are no more children it stops
